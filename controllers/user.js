...
const register = async (req, res) => {
    const { name, email, age, gender, password, target, lat, long, motto } = req.body;
...
const login = async (req, res) => {
    ...
    if (targetUser) {
        if (bcryptjs.compareSync(password, targetUser.password)) {
            const token = jwt.sign({ id: targetUser.id, name: targetUser.name }, "datingApp", { expiresIn: 3600 });
